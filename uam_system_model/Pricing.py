...
class PricingOptimizer():
    def __init__(
        ...
        pax_arr['passenger_arrival_time_slot'] = np.ceil(pax_arr['passenger_arrival_time_s'] / self.time_resolution / 60).astype(int)
        ...
        self.pax_arr_grouped = pax_arr_grouped.sort_values(by=["passenger_arrival_time_slot"]).reset_index(drop=True)

        self.flight_time_matrix =  np.ceil(self.network.flight_time * 5 / self.time_resolution)
        ...
        for idx, row in self.pax_arr_grouped.iterrows():
            all_tasks.append(
                FlightTask(
                    ...
                    duration=self.flight_time_matrix[row['origin_vertiport_id'], row['destination_vertiport_id']],
        ...
        self.nodes, self.supply, self.edges, self.di_bar, self.c = assignment_network.populate_network()
    ...
    def optimize(
        ...
        m._x_inverse_vars = m.addVars(self.edges, vtype=GRB.CONTINUOUS, name="x_ij_inverse", lb=0)
        ...
        for i in range(len(non_zero_indices)):
            ...
            m.addConstr(
                m._x_vars[self.edges[idx]] * 4 >= m._theta_uam[i] * di_bar_selected_x[i], f"cap_{i}"
            )


        operating_cost = quicksum(m._x_vars[self.edges[i]] * flight_cost_uam[i_non_zero] for i_non_zero, i in zip(range(len(di_bar_selected_x)), non_zero_indices))
        ...
        prices['fare'] = prices.apply(lambda row: self.calc_fare(row, value_of_time, v_i_bar_uber, t_i_uam), axis=1)
