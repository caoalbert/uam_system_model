...
# flight hour + flight cycle based cost calculation with FATO capacity constraint
class PricingOptimizer:
    ...
    def optimize(
        ...
        self.flight_time_matrix = np.ceil(
            self.network.flight_time / self.time_resolution
        )
        ...
        for idx, row in self.pax_arr_grouped.iterrows():
            all_tasks.append(
                FlightTask(
                    ...
                    num_pax=row["counts"],
                    uam_flight_time=self.network.flight_time,
        ...
        for idx, row in self.pax_arr_grouped.iterrows():
            ...
            first_last_mile_cost.append(
                first_and_last_cost[origin, destination, time]
            )
            ...
            flight_hour = uam_flight_time[origin, destination] / 60
            flight_cost_uam.append(
                cost_per_fh * flight_hour + cost_per_fc
            )
        ...
        # Aircraft seat capacity constraints
        for i in range(len(non_zero_indices)):
            idx = non_zero_indices[i]
            m.addConstr(
                m._x_vars[self.edges[idx]] * num_seats
        ...
        # FATO capacity constraints 
        vertiport_usage = {}  # Dictionary to store (vertiport, time) -> [list of Gurobi variables]
        cap = 0
        
        if isinstance(fato_capacity, int):
            fato_capacity = [fato_capacity for _ in range(len(self.network.vertiport_dict))]
        ...
        for (v_id, t_slot), vars_list in vertiport_usage.items():
            cap = fato_capacity[v_id]
            m.addConstr(
                quicksum(vars_list) <= cap,
                name=f"FATO_Cap_V{v_id}_T{t_slot}"
            )
            
        del cap, vertiport_usage  # Deallocation
        ...
        operating_cost = quicksum(
            ...
            * (
                cost_per_fh * di_bar_selected_negative[i_zero]
                - cost_per_fc
            )
        ...
        if repo_flights.empty:
            # pandas 3.x: apply() on empty DataFrame returns empty DataFrame,
            # not empty Series — assign typed empty columns directly.
            repo_flights["repo_flight_time"]    = pd.Series(dtype=float)
            repo_flights["time_slot"]           = pd.Series(dtype=float)
            repo_flights["repositioning_hour"]  = pd.Series(dtype=float)
            repo_flights["cost"]                = pd.Series(dtype=float)
        else:
            ...
            repo_flights["repositioning_hour"] = repo_flights.apply(
                lambda row: uam_flight_time[
                    int(row["origin_vertiport_id"]), int(row["destination_vertiport_id"])
                ] / 60,
                axis=1,
            )
            repo_flights["cost"] = repo_flights["repositioning_hour"].apply(
                lambda x: cost_per_fh * x + cost_per_fc if x > 0 else 0
            )
...
class FlightTask:
    def __init__(
        ...
        self.uam_flight_time_matrix = uam_flight_time
        self.min_arrrival_slot = min_arrrival_slot # aircraft must arrive before n time slots at maximum
    ...
    def next_task(self, next_task):
        ...
        if (
            ...
            and next_task_start_time - ready_time <= self.min_arrrival_slot
        ):
            return True
        else:
            return False
        ...
        for i in range(number_of_tasks):
            for j in range(i + 1, number_of_tasks):
                ...
                if if_connect:
                    ...
                    repo_hour = self.list_of_tasks[i].uam_flight_time_matrix[
                        self.list_of_tasks[i].destination, self.list_of_tasks[j].origin
                    ] / 60 # repositioning flight time in hours
                    di_bar.append(-1 * repo_hour)
