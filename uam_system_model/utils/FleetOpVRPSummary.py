...
class FleetOpVRPSummary(FleetOpVRP):
    def __init__(self, StarNetwork, policy, non_linear_battery_charging=True):
        ...
        if non_linear_battery_charging:
            self.cs = np.cumsum(np.array([0, 0.0129,0.0133,0.0137,0.0142,0.0147,
    ...
    def get_summary_statistics(self):
        ...
        num_pads_at_vertiport = {self.network.vertiport_dict_inv[i]: parking_pads[parking_pads['vertiport_idx'] == i]['num_pads'].values[0] for i in range(len(self.network.vertiport_dict))}

        max_takeoff_per_hour = self._infer_num_tlof()
        max_takeoff_per_hour_at_vertiport = {self.network.vertiport_dict_inv[i]: max_takeoff_per_hour[max_takeoff_per_hour['location'] == i]['count'].values[0] for i in range(len(self.network.vertiport_dict))}
        ...
        total_number_of_flights = repositioning_flights + self.policy['tour_length'].sum()
        energy_consumption_counted = (self.policy['q']-self.policy['p']).values.sum() + (self.policy['q2']-self.policy['p2']).values.sum()
        last_entries = self.policy.groupby("cluster_id").tail(1)
        energy_consumption_not_counted = (100 - last_entries['q2'].values).sum()
        energy_consumption = energy_consumption_counted + energy_consumption_not_counted / 100 * 160 
        ...
        for idx, row in self.policy.iterrows():
            ...
            for i in range(len(sequence)-1):
                ...
                revenue_aircraft_miles += self.network.flight_distance_matrix[origin][destination]
        ...
        summary = {
            ...
            "percentage_repositioning_flights": round(repositioning_flights / total_number_of_flights,4),
    ...
    def _append_column(self):
        ...
        self.policy['next_origin'] = self.policy.groupby('cluster_id')['origin'].shift(-1)
        self.policy['next_flight_start_time'] = self.policy.groupby('cluster_id')['time'].shift(-1)
        self.policy['next_origin'] = self.policy['next_origin'].fillna(999)
        self.policy['repo_flight_duration'] = self.policy.apply(
            lambda row: self.network.flight_time[int(row['destination']), int(row['next_origin'])] * 5
            if row['next_origin'] != 999
            else 0,
            axis=1
        )
        self.policy['repo_flight_miles'] = self.policy.apply(
            lambda row: self.network.flight_distance_matrix[int(row['destination']), int(row['next_origin'])]
            if row['next_origin'] != 999
            else 0,
            axis=1
        )
        self.policy['is_repo']= self.policy.apply(
            lambda row: 1 if row['destination'] != row['next_origin'] and  row['next_origin'] != 999 else 0,
            axis=1
        )
        ...
        self.policy['reposition_start_time'] = self.policy['time'] + self.policy['duration'] + self.policy['charge_time_before_repo']
        self.policy['reposition_end_time'] = self.policy['reposition_start_time'] + self.policy['repo_flight_duration']
    ...
    def _infer_num_pads(self):
        ...
        for index, row in self.policy.iterrows():
            interval_1 = {
                ...
                "end": row["reposition_start_time"] if pd.notna(row["reposition_start_time"]) else 10000,
                "location": row["destination"],
            }
            interval_2 = {
                "start": row["reposition_end_time"],
                "end": row["next_flight_start_time"] if pd.notna(row["next_flight_start_time"]) else 10000,
                "location": row["next_origin"] if row["next_origin"] != 999 else row["destination"],
            }
    ...
    def _infer_num_tlof(self):
        df = pd.DataFrame(
            columns=["hour", "location"]
        )
        for index, row in self.policy.iterrows():
            if row['is_repo']:
                df = df.append({"hour": row["reposition_hour"], "location": row["next_origin"]}, ignore_index=True)
            df = df.append({"hour": row["revenue_flight_hour"], "location": row["origin"]}, ignore_index=True)
            sequence = row["tour_sequence"].split("-")
            if len(sequence) < 2:
                continue
