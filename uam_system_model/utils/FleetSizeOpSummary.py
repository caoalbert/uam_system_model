...
class FleetSizeOpSummary(FleetSizeOptimizer):
    ...
    def get_summary_statistics(self):
        ...
        fleet_size = self.all_c.sum(axis=0)[0] + self.all_n.sum(axis=0)[0] + self.all_u.sum(axis=0)[0]
        ...
        flight_demand = schedule[schedule['od'].isin([f"LAX_{self.network.vertiport_dict_inv[i]}", 
                                                        f"{self.network.vertiport_dict_inv[i]}_LAX"])].shape[0]
        ...
        for idx, row in self.specificu.iterrows():
            ...
            energy_consumption += amount * self.energy_consumption[t][i][j] * 0.025 * 160 # Convert to kWh
        ...
        revenue_aircraft_miles = flight_demand * self.network.flight_distance_matrix[0][self.i]
    ...
    def plot_aircraft_state(self):
        ...
        sns.lineplot(np.arange(x1), self.all_c.sum(axis=0)[x0:x1], label="Charging", ax=ax[0], color=color[0])
        sns.lineplot(np.arange(x1), self.all_u.sum(axis=0)[x0:x1], label="In Flight", ax=ax[0], color=color[1])
        sns.lineplot(np.arange(x1), self.all_n.sum(axis=0)[x0:x1], label="Idling", ax=ax[0], color=color[2])
        sns.lineplot(
            np.arange(x1),
            (
        ...
        sns.lineplot(np.arange(x1), self.hub_c.sum(axis=0)[x0:x1], label="Charging", ax=ax[1], color=color[0])
        sns.lineplot(np.arange(x1), self.hub_u.sum(axis=0)[x0:x1], label="In Flight", ax=ax[1], color=color[1])
        sns.lineplot(np.arange(x1), self.hub_n.sum(axis=0)[x0:x1], label="Idling", ax=ax[1], color=color[2])
        sns.lineplot(
            np.arange(x1),  
            (
        ...
        sns.lineplot(np.arange(x1), self.spoke_c.sum(axis=0)[x0:x1], label="Charging", ax=ax[2], color=color[0])
        sns.lineplot(np.arange(x1), self.spoke_u.sum(axis=0)[x0:x1], label="In Flight", ax=ax[2], color=color[1])
        sns.lineplot(np.arange(x1), self.spoke_n.sum(axis=0)[x0:x1], label="Idling", ax=ax[2], color=color[2])
        sns.lineplot(
            np.arange(x1),
            (
    ...
    def _compute_system_states(self):
        self.all_c, self.all_u, self.all_n = self._compute_states(self.specificc, self.specificu, self.specificn)
    ...
    def _parse_policy(self, policy):
        ...
        n[['t','i','k']] = n['Variable'].apply(lambda x: x.split('[')[1].replace(']','').split(',')).tolist()
        n['amount'] = n['Value'].astype(int)
        n = n[['t','i','k','amount']].astype(int).sort_values(by=['t','i','k']).reset_index(drop=True)

        u[['t','i','j','k']] = u['Variable'].apply(lambda x: x.split('[')[1].replace(']','').split(',')).tolist()
        u['amount'] = u['Value'].astype(int)
        u = u[['t','i','j','k','amount']].astype(int).sort_values(by=['t','i','j','k']).reset_index(drop=True)

        c[['t','i','x','y']] = c['Variable'].apply(lambda x: x.split('[')[1].replace(']','').split(',')).tolist()
        c['amount'] = c['Value'].astype(int)
        c = c[['t','i','x','y','amount']].astype(int).sort_values(by=['t','i','x','y']).reset_index(drop=True)
